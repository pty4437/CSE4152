...
cudaError_t Fibonacci_GPU(int *h_Fibo_n, int *h_Fibo_v, int *h_Fibo_v_GPU)
{
    ...
    cudaMalloc(&d_Fibo_n, size);
    cudaMalloc(&d_Fibo_v, size);
    cudaMemcpy(d_Fibo_n, h_Fibo_n, size, cudaMemcpyHostToDevice);

    CHECK_TIME_START_GPU();

    Fibonacci_Kernel << < dimGrid, dimBlock >> > (d_Fibo_n, d_Fibo_v);

    CHECK_TIME_END_GPU(device_time)

    cudaGetLastError();
    cudaDeviceSynchronize();
    cudaMemcpy(h_Fibo_v_GPU, d_Fibo_v, size, cudaMemcpyDeviceToHost);
