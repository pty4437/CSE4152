...
#define BLOCK_SIZE (1 << 7)													// CUDA 커널 thread block 사이즈
...
__global__ void Sum_n_elements_Kernel(IN int *d_ArrayElements, OUT int *d_SumOfArrayElements, int N, int Nf) {
    ...
    d_SumOfArrayElements[idx] = 0;
    for (int k = idx - Nf; k <= idx + Nf; k++) {
        if (k < 0 || k >= N) 
            continue;
        d_SumOfArrayElements[idx] += d_ArrayElements[k];
    }
}
...
cudaError_t Sum_n_elements_GPU(IN int *p_ArrayElements, OUT int *p_SumOfElements_GPU, int Nf) {
    ...
    cudaMemcpy(d_Array, p_ArrayElements, size, cudaMemcpyHostToDevice);
    ...
    cudaMemcpy(p_SumOfElements_GPU, d_Sum, size, cudaMemcpyDeviceToHost);
