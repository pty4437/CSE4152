...
__global__ void Mat_Vec_Multiply_Kernel(float* d_VecY, float* d_VecX, float* d_MatA, int Vec_Size) {
	...
	for (int j = 0; j < Vec_Size; j++) {
		...
		for (int k = 0; k < Vec_Size; k++) 
			sum += d_MatA[j * Vec_Size + k] * d_VecX[idx * Vec_Size + k];
		
		d_VecY[idx * Vec_Size + j] = sum;
	}
}

void Mat_Vec_Multiply_GPU(float* p_VecX, float* p_MatA, float* p_VecY_G) {
	CHECK_TIME_INIT_GPU()
	float* VecY, * VecX, * MatA;
	dim3 block(BLOCK_SIZE, 1);
	dim3 grid(ELEMENT_SIZE / BLOCK_SIZE, 1);

	cudaMalloc(&VecX, sizeof(float) * n * ELEM_PER_VECTOR);
	cudaMalloc(&VecY, sizeof(float) * n * ELEM_PER_VECTOR);
	cudaMalloc(&MatA, sizeof(float) * ELEM_PER_VECTOR * ELEM_PER_VECTOR);

	cudaMemcpy(VecX, p_VecX, sizeof(float) * n * ELEM_PER_VECTOR, cudaMemcpyHostToDevice);
	cudaMemcpy(MatA, p_MatA, sizeof(float) * ELEM_PER_VECTOR * ELEM_PER_VECTOR, cudaMemcpyHostToDevice);	

	CHECK_TIME_START_GPU()
		Mat_Vec_Multiply_Kernel << < grid, block >> > (VecY, VecX, MatA, ELEM_PER_VECTOR);
	CHECK_TIME_END_GPU(device_time)

	cudaDeviceSynchronize();
	cudaMemcpy(p_VecY_G, VecY, sizeof(float) * n * ELEM_PER_VECTOR, cudaMemcpyDeviceToHost);

	cudaFree(VecX); cudaFree(VecY); cudaFree(MatA);
